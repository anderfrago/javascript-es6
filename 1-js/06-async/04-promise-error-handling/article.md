...
\`\`\`js run _!_ fetch\('[https://no-such-server.blabla](https://no-such-server.blabla)'\) // Promesa rechazada _/!_ .then\(response =&gt; response.json\(\)\) .catch\(err =&gt; alert\(err\)\) // TypeError: failed to fetch \(El texto puede variar, dependiendo del error\)
...
\`\`\`js run new Promise\(\(resolve, reject\) =&gt; { resolve\("ok"\); }\).then\(\(result\) =&gt; { _!_ throw new Error\("Whoops!"\); // rechaza la promesa _/!_ }\).catch\(alert\); // Error: Whoops!
...
\`\`\`js run // Ejecución: catch -&gt; then new Promise\(\(resolve, reject\) =&gt; {
...
\`\`\`js untrusted run refresh new Promise\(function\(\) { noSuchFunction\(\); // Aquí hay un error \(no existe la función\) }\) .then\(\(\) =&gt; { // manejador de una o más promesas exitosas }\); // sin .catch al final!
