...
Cubrimos completamente `method`, `headers` y `body` en el capítulo .

La opción `signal` está cubierta en .
...
`````smart header="La política Referrer no es solo para```fetch`" La política Referrer, descrita en la [especificación](https://w3c.github.io/webappsec-referrer-policy/), no es solo para`fetch\`, sino más global.
